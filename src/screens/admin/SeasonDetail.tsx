...
export default function SeasonDetailPage() {
  ...
  const title = seasonYear
    ? ADMIN_GAME_EDITOR.pageTitle(seasonYear)
    : "Season";
  ...
  return (
    <Page>
      ...
      />
      <Link to="/admin" className="btn btn-ghost btn-sm" style={{ marginBottom: "var(--space-4)" }}>
  ...
  return (
    <div className="section">
      ...
      <Table
        ...
        rowKey={(g) => g.id}
        renderRow={(g) => (
          <GameRow key={g.id} game={g} onRefresh={onRefresh} />
        )}
      />
    </div>
  );
